...
{
    public partial class QuotingInfoMultiLoaderListForm : Form
    {
        private KRXDBQuotingInfoManager infoManager = new KRXDBQuotingInfoManager();
        private Form parentForm = null;
        string senderServerId;
        ServerInfoManager serverInfoManager;
        public QuotingInfoMultiLoaderListForm(string senderServerId)
        {
        ...
        public void Init()
        {
            ...
            comboBoxServer.DisplayMember = "serverId";
            foreach (ServerInfoManager serverInfoManager in ServerManager.Instance.GetServerInfoManagerList())
            {
                comboBoxServer.Items.Add(serverInfoManager);
                if(senderServerId == serverInfoManager.serverId)
                {
                    comboBoxServer.SelectedIndex = comboBoxServer.Items.IndexOf(serverInfoManager);
                }
            }
        }
        ...
        {
            serverInfoManager = (ServerInfoManager)comboBoxServer.SelectedItem;
            int count = 0;
            ...
            for (int i = 0; i < iGridQuotingInfo.Rows.Count; i++)
            {
                ...
                if (row.Selected)
                {
                    ...
                    {
                        serverInfoManager.KrxdbInfoManager.UpdateQuotingInfo(info, true);
                    }
                }
            }
